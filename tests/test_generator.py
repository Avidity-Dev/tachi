...
This module tests the AzureGenerator class which generates GitHub Actions workflows
and Azure Container Apps configurations from project specifications.
"""
import pytest
...
class TestAzureGenerator:
    ...
    Tests workflow generation, container app configuration generation, and proper
    file structure creation for all deployment strategies.
    """
    ...
    def test_workflow_content_validation(self, temp_dir, sample_config, template_dir):
        ...
        assert "test-app" in prod_workflow
        assert "api" in prod_workflow
...
class TestTemplateRendering:
    ...
    Tests the custom Jinja2 delimiter configuration used to avoid conflicts
