...
from tachi.config import ProjectConfig, Service
...
class AzureGenerator:
    ...
    def _generate_workflow(self, config: ProjectConfig, workflow_name: str, output_dir: Path) -> None:
        """Generate a single workflow file (stub for now)"""
        # TODO: Implement template rendering when templates are ready
        workflow_content = f"""# {workflow_name} workflow for {config.name}
# Generated by tachi
# TODO: Implement actual workflow template
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to Azure
        run: echo "Deploying {config.name} using {config.strategy} strategy"
"""
    ...
    def _generate_container_configs(self, config: ProjectConfig, output_dir: Path) -> None:
        """Generate container app configuration files (stub for now)"""
        for service in config.services:
            # TODO: Implement template rendering when templates are ready
            container_config = {
                "name": f"{config.name}-{service.name}",
                "properties": {
                    "configuration": {
                        "ingress": {
            ...
            config_file = output_dir / f"{service.name}.json"
            config_file.write_text(json.dumps(container_config, indent=2))
