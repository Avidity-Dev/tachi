...
def generate(config: Optional[str], output: str, dry_run: bool, force: bool):
    ...
    if config:
        with click.progressbar(length=5, label="Generating files") as bar:
            ...
            click.echo(
                f"   🚀 Strategy: {click.style(project_config.strategy, fg='cyan')}"
            )
            click.echo(
                f"   🔧 Services: {click.style(str(len(project_config.services)), fg='green')} "
                + f"({', '.join(s.name for s in project_config.services)})"
            )
            click.echo(
                f"   📂 Output: {click.style(str(output_dir.absolute()), fg='yellow')}"
            )
        ...
        while True:
            ...
            dockerfile = click.prompt(
                "  Dockerfile path", default="Dockerfile", type=str
            )
...
def validate(config: str, verbose: bool):
    ...
    for service in project_config.services:
        ...
        click.echo(
            f"     Scaling:    {service.min_replicas}-{service.max_replicas} replicas"
        )
        if verbose:
